...
def test_addition(sample_domain):
    sample1 = ExperimentSample(_input_data={"x1": 1}, _output_data={"y1": 3})
    sample2 = ExperimentSample(_input_data={"x2": 2}, _output_data={})
...
def test_equality(sample_domain):
    sample1 = ExperimentSample(_input_data={"x1": 1}, _output_data={"y1": 3})
    sample2 = ExperimentSample(_input_data={"x1": 1}, _output_data={"y1": 3})
    assert sample1 == sample2


def test_replace_nan(sample_domain):
    sample = ExperimentSample(
        _input_data={"x1": float("nan")}, _output_data={"y1": 3}
    )
...
def test_round(sample_domain):
    sample = ExperimentSample(
        _input_data={"x1": 1.2345}, _output_data={"y1": 3.6789}
    )
    sample.round(2)
...
def test_to_multiindex(sample_domain):
    sample = ExperimentSample(_input_data={"x1": 1}, _output_data={"y1": 3})
...
def test_to_numpy(sample_domain):
    sample = ExperimentSample(
        _input_data={"x1": 1, "x2": 2}, _output_data={"y1": 3}
    )
...
def test_to_numpy_with_nan(sample_domain):
    ...
    """
    sample = ExperimentSample(
        _input_data={"x1": float("nan"), "x2": 2}, _output_data={"y1": 3}
    )
    input_array, output_array = sample.to_numpy()
