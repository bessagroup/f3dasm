...
def main(sim_info):
    ...
    ELSE_SubField = ELSE_Field.getSubset(region=entireRVE_elSet, position=WHOLE_ELEMENT)
    #
    if isinstance(ELSE_SubField.values[0].data, float):
        ...
        av_ELSE = numpy.zeros((totalNumFrames, len(ELSE_SubField.values[0].data)))
        ...
        for iFrame_step in range(0, numFrames):
            ...
            if isinstance(ELSE_SubField.values[0].data, float):
                ...
                for i in range(0, len(ELSE_SubField.values)):
                    av_ELSE[iFrame] = av_ELSE[iFrame] + ELSE_SubField.values[i].data
                #
                ...
                for j in range(0, len(ELSE_SubField.values[0].data)):
                    for i in range(0, len(ELSE_SubField.values)):
                        av_ELSE[iFrame][j] = av_ELSE[iFrame][j] + ELSE_SubField.values[i].data[j]
                    #
            ...
            Green_strain[iFrame, :, :] = 0.5 * (
                numpy.dot(numpy.transpose(defGrad[iFrame, :, :]), defGrad[iFrame, :, :]) - numpy.identity(2)
            )
