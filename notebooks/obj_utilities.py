...
def _get_variables(variables):
    return np.concatenate([v.numpy().ravel() if not isinstance(v, np.ndarray) else v.ravel() for v in variables])


def train_lbfgs(model, func_obj, max_iterations, path="", n_saves=1, conv_criteria=False, limit=0.01, **kwargs):
    """ """
...
def train_tf_optimizer(
    model, func_obj, optimizer, max_iterations, path="", n_saves=1, conv_criteria=False, limit=0.01, **kwargs
):
    """ """
    func = convert_autograd_to_tensorflow(func_obj.ask)  # So that gradients can flow to the models
    model(None)  # Build the model
