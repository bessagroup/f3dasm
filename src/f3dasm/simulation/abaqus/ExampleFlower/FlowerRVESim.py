...
def main(sim_info):
    ...
    sketch = model.ConstrainedSketch(name='__profile__', sheetSize=10.50000,)
    # g, v, d, c = s.geometry, s.vertices, s.dimensions, s.constraints
    sketch.setPrimaryObject(option=STANDALONE)
    sketch.rectangle(point1=(RVEcenter[0]-Lx/2,RVEcenter[1]-Ly/2), point2=(RVEcenter[0]+Lx/2, RVEcenter[1]+Ly/2))
    ...
    # edges
    s = part.edges
    edgesLEFT = s.getByBoundingBox(RVEcenter[0]-Lx/2-delta, RVEcenter[1]-Ly/2-delta, 0,
                                   RVEcenter[0]-Lx/2+delta, RVEcenter[1]+Ly/2+delta, 0)
    part.Set(edges=edgesLEFT, name='LeftEdge')
    edgesRIGHT = s.getByBoundingBox(RVEcenter[0]+Lx/2-delta, RVEcenter[1]-Ly/2-delta, 0,
                                    RVEcenter[0]+Lx/2+delta, RVEcenter[1]+Ly/2+delta, 0)
    part.Set(edges=edgesRIGHT, name='RightEdge')
    edgesTOP = s.getByBoundingBox(RVEcenter[0]-Lx/2-delta, RVEcenter[1]+Ly/2-delta, 0,
                                  RVEcenter[0]+Lx/2+delta, RVEcenter[1]+Ly/2+delta, 0)
    part.Set(edges=edgesTOP, name='TopEdge')
    edgesBOT = s.getByBoundingBox(RVEcenter[0]-Lx/2-delta, RVEcenter[1]-Ly/2-delta, 0,
                                  RVEcenter[0]+Lx/2+delta, RVEcenter[1]-Ly/2+delta, 0)
    part.Set(edges=edgesBOT, name='BotEdge')
    # vertices
    v = part.vertices
    vertexLB = v.getByBoundingBox(RVEcenter[0]-Lx/2-delta, RVEcenter[1]-Ly/2-delta, 0,
                                  RVEcenter[0]-Lx/2+delta, RVEcenter[1]-Ly/2+delta, 0)
    part.Set(vertices=vertexLB, name='VertexLB')
    vertexRB = v.getByBoundingBox(RVEcenter[0]+Lx/2-delta, RVEcenter[1]-Ly/2-delta, 0,
                                  RVEcenter[0]+Lx/2+delta, RVEcenter[1]-Ly/2+delta, 0)
    part.Set(vertices=vertexRB, name='VertexRB')
    vertexRT = v.getByBoundingBox(RVEcenter[0]+Lx/2-delta, RVEcenter[1]+Ly/2-delta, 0,
                                  RVEcenter[0]+Lx/2+delta, RVEcenter[1]+Ly/2+delta, 0)
    part.Set(vertices=vertexRT, name='VertexRT')
    vertexLT = v.getByBoundingBox(RVEcenter[0]-Lx/2-delta, RVEcenter[1]+Ly/2-delta, 0,
                                  RVEcenter[0]-Lx/2+delta, RVEcenter[1]+Ly/2+delta, 0)
    ...
    if mat_name == 'Arruda':
        ...
        material.Hyperelastic(materialType=ISOTROPIC, testData=OFF, type=ARRUDA_BOYCE,
                              volumetricResponse=VOLUMETRIC_DATA, table=((166.0, 2.8, 0.0025), ))
        model.HomogeneousSolidSection(name='Section-1', material='Arruda', thickness=None)
        part.SectionAssignment(region=part.sets['Phase_1'], sectionName='Section-1', offset=0.0,
        ...
        material.Density(table=((1e-21, ), ))
        material.Expansion(table=((5.8e-05, 0.0), (5.8e-05, 200.0)), zero=120.0, temperatureDependency=ON)
        material.Hyperelastic(materialType=ISOTROPIC, testData=OFF, type=NEO_HOOKE,
                              volumetricResponse=VOLUMETRIC_DATA, table=((961.538, 0.0005), ))
    ...
    model.Equation(name='LB_RT_1', terms=((1, 'NodeRT', 1), (-1,'NodeLB',1), (-1*Lx, 'Ref-R', 1), (-1*Ly, 'Ref-T', 1)))
    model.Equation(name='LB_RT_2', terms=((1, 'NodeRT', 2), (-1,'NodeLB',2), (-1*Lx, 'Ref-R', 2), (-1*Ly, 'Ref-T', 2)))

    model.Equation(name='LT_RB_1', terms=((1, 'NodeRB', 1), (-1,'NodeLT',1), (-1*Lx, 'Ref-R', 1), (1*Ly, 'Ref-T', 1)))
    model.Equation(name='LT_RB_2', terms=((1, 'NodeRB', 2), (-1,'NodeLT',2), (-1*Lx, 'Ref-R', 2), (1*Ly, 'Ref-T', 2)))
    ...
    if len(RightEdge_nodes_sorted) ==len(LeftEdge_nodes_sorted):
        for ii in range(1, len(RightEdge_nodes_sorted)-1):
            model_assembly.SetFromNodeLabels(name='LEFT_' + str(ii), nodeLabels=(('FinalRVE', tuple([LeftEdge_nodes_sorted[ii].label])),), unsorted=True)
            model_assembly.SetFromNodeLabels(name='RIGHT_' + str(ii), nodeLabels=(('FinalRVE', tuple([RightEdge_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1,3):
                model.Equation(name='LEFT_RIGHT_' + str(ii) + '_' + str(jj),
    ...
    if len(TopEdge_nodes_sorted) ==len(BotEdge_nodes_sorted):
        for ii in range(1, len(TopEdge_nodes_sorted)-1):
            model_assembly.SetFromNodeLabels(name='TOP_' + str(ii), nodeLabels=(('FinalRVE', tuple([TopEdge_nodes_sorted[ii].label])),), unsorted=True)
            model_assembly.SetFromNodeLabels(name='BOT_' + str(ii), nodeLabels=(('FinalRVE', tuple([BotEdge_nodes_sorted[ii].label])),), unsorted=True)
            for jj in range(1,3):
                model.Equation(name='TOP_BOT_' + str(ii) + '_' + str(jj),
                                   terms=((1, 'TOP_' + str(ii), jj), (-1, 'BOT_' + str(ii), jj), (-1*Ly, 'Ref-T', jj)))
    else:
        print "the number of nodes between the two sides are not the same"

    ## create step
    model.StaticStep(name='Step-1', previous='Initial', stabilizationMagnitude=0.0002, stabilizationMethod=DISSIPATED_ENERGY_FRACTION,
                     continueDampingFactors=False, adaptiveDampingRatio=None, initialInc=0.02, maxInc=0.02, nlgeom=ON)
    model.Temperature(name='Predefined Field-1', createStepName='Initial', region=model_assembly.instances['FinalRVE'].sets['Phase_1'],
                      distributionType=UNIFORM, crossSectionDistribution=CONSTANT_THROUGH_THICKNESS, magnitudes=(120.0, ))
    ## create Final-outputs
    model.fieldOutputRequests['F-Output-1'].setValues(variables=( 'S', 'E', 'LE', 'ENER', 'ELEN', 'ELEDEN', 'EVOL', 'IVOL'), timeInterval=0.1)
    model.FieldOutputRequest(name='F-Output-2', createStepName='Step-1', variables=('U', 'RF'), timeInterval=0.1)
    model.historyOutputRequests['H-Output-1'].setValues(variables=('ALLAE', 'ALLCD', 'ALLIE', 'ALLKE', 'ALLPD', 'ALLSE', 'ALLWK', 'ETOTAL'), timeInterval=0.1)
    ...
    model.DisplacementBC(name='E_11', createStepName='Step-1',
                             region=model_assembly.sets['Ref-R'], u1=0.725238, u2=UNSET, ur3=UNSET, amplitude=UNSET, fixed=OFF,
                             distributionType=UNIFORM, fieldName='', localCsys=None)
    model.DisplacementBC(name='E_12', createStepName='Step-1',
                             region=model_assembly.sets['Ref-R'], u1=UNSET, u2=0.153468, ur3=UNSET, amplitude=UNSET, fixed=OFF,
                             distributionType=UNIFORM, fieldName='', localCsys=None)
    model.DisplacementBC(name='E_21', createStepName='Step-1',
                             region=model_assembly.sets['Ref-T'], u1=0.153468, u2=UNSET, ur3=UNSET, amplitude=UNSET, fixed=OFF,
                             distributionType=UNIFORM, fieldName='', localCsys=None)
    model.DisplacementBC(name='E_22', createStepName='Step-1',
                             region=model_assembly.sets['Ref-T'], u1=UNSET, u2=-0.118837, ur3=UNSET, amplitude=UNSET, fixed=OFF,
                             distributionType=UNIFORM, fieldName='', localCsys=None)
